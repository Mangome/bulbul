...
- **Don't** improvise motion. Durations must come from the 80/120/200/300ms scale, easings from the four tokenized curves, enter distances from {4px, 8px}. No spring parameters, no stagger choreography (WelcomePage first-screen excepted), no layout animations, no counting/rolling numbers. Motion values live in the token matrix only; see docs/design-proposals/proposal-a/design-language.md for the binding motion spec.
