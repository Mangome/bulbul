...
为 TIFF/EP 格式的 RAW 文件（Canon CR2、Sony ARW、Adobe DNG、Olympus ORF、Panasonic RW2、Pentax PEF）提供统一的嵌入 JPEG 提取和 Exif 元数据解析能力。这些格式共享相同的 TIFF/EP 容器结构，因此可复用通用的 `extract_largest_jpeg()` 和 `parse_exif()` 函数。
