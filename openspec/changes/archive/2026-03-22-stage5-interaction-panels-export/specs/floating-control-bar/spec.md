...
FloatingControlBar SHALL 在主窗口底部居中悬浮显示，包含缩放滑块（范围 10%~300%）、`[-]` 缩小按钮、`[+]` 放大按钮、当前缩放百分比文本。所有缩放操作 SHALL 同步到 useCanvasStore。
...
控制栏 SHALL 采用 pill 形状容器，半透明白色背景 + backdrop-filter: blur(20px)，各区域以竖线分隔。
