...
系统 SHALL 在主窗口左侧显示悬浮分组列表面板（FloatingGroupList），以 `position: fixed` 定位于画布上方。面板 SHALL 采用白色半透明背景（rgba 255,255,255,0.94）+ backdrop-filter: blur(20px) + 圆角 12px + 柔和阴影。

#### Scenario: 分组列表正常展示

- **WHEN** 处理完成后分组数据可用
- **THEN** 左侧面板显示所有分组，每项包含：代表图缩略图（50×50, 圆角 6px）、分组名称、图片数量、平均相似度百分比
