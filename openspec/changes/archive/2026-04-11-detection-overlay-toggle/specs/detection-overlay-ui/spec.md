...
系统 SHALL 提供纯函数 `drawDetectionOverlay(ctx, boxes, displayWidth, displayHeight)` 在 Canvas 2D 上下文中绘制鸟类检测框。调用时 ctx 已处于图片项的局部坐标系（原点在图片左上角）。当 `zoomLevel < 0.4` 时，`CanvasImageItem.draw()` SHALL 跳过检测框绘制调用。

#### Scenario: 绘制主框（最高置信度）
- **WHEN** `boxes` 包含多个检测框，且 `zoomLevel >= 0.4`
- **THEN** 系统 SHALL 以 `#22C55E`（绿色）绘制置信度最高的框，线宽 2px

#### Scenario: 绘制副框（非最高置信度）
- **WHEN** `boxes` 中存在置信度低于最大值的框，且 `zoomLevel >= 0.4`
...
#### Scenario: 缩放低于阈值不绘制
- **WHEN** `zoomLevel < 0.4`
- **THEN** `CanvasImageItem.draw()` SHALL 跳过 `drawDetectionOverlay()` 调用，即使 `detectionVisible` 为 `true` 且 `detectionBoxes` 非空
