...
**Reason**: PixiJS Sprite 渲染被 Canvas 2D `ctx.drawImage()` 替代
**Migration**: 使用 `CanvasImageItem.setImage(imageBitmap)` 替代 `setTexture(texture)`，使用 `draw(ctx, zoom, now)` 替代 PixiJS 自动渲染
